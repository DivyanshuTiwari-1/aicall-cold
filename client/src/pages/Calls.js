...
const Calls = () => {
    ...
    if (error) return <div className = "text-center py-12" > < p className = "text-red-600" > Failed to load calls < /p></div > ;
    ...
    return ( <
        ...
        onChange = {
            (e) => setOutcomeFilter(e.target.value) }
        className = "input-field" >
        <
        ...
        div className = "space-y-4" > {
            calls ? .calls ? .length === 0 ? ( <
                ...
                calls ? .calls ? .map((call) => ( <
                    ...
                    <
                    div className = "flex items-center space-x-4" > { getStatusIcon(call.status) } <
                    div >
                    <
                    h3 className = "font-medium text-gray-900" > { call.contactName } < /h3> <
                    p className = "text-sm text-gray-500" > { call.company }• { call.phone } < /p> <
                    ...
                    <
                    span className = { `inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(call.status)}` } > { call.status.replace('_', ' ') } <
                    /span> {
                        call.outcome && ( <
                    ...
                    /div> <
                    p className = "text-sm text-gray-500" > { formatDuration(call.duration) }• { formatDate(call.createdAt) } <
                    /p> <
                    /div>
