import { CloudArrowUpIcon, UserPlusIcon, XMarkIcon } from "@heroicons/react/24/outline";
...
const UploadContactsModal = ({ isOpen, onClose }) => {
    ...
    const importMutation = useMutation({
        mutationFn: (data) => contactsAPI.importContacts(data),
        onSuccess: (result) => {
            toast.success("Contacts imported successfully!");
            setUploadResults(result);
            queryClient.invalidateQueries({ queryKey: ['contacts'] });
            // Don't close modal immediately to show results
        },
        onError: (error) => {
            toast.error(
    ...
    const handleManualSubmit = (e) => {
        ...
        if (!manualContact.first_name.trim() || !manualContact.phone.trim()) {
